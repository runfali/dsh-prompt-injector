...
```bash
npm test                          # 下面三套按序全跑
node --test test/entry.test.mjs   # 真宿主入口加载 + engines 区间 + 三处键集合一致（先跑）
...
- `src/index.js` — 插件入口：`settings.installSection` 接线（dsh 0.1.2-alpha.3+，0.1.5-rc.1 无变化）+ `agent/pre-step` 注入 + agent hooks/backfill。
