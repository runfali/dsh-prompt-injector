  ...
  factory: (require) => {
    ...
    function parseFieldValue(field, raw) {
      if (field.type === "list") {
        ...
        // 注意：不过滤空行——用户点击「添加」产生的新行（title/text 均空）
        // 必须保留在列表里等待填写；host 注入侧已过滤空正文行。
        const cleaned = raw
          .map((p) => ({
            id: String((p && p.id) || ""),
            ...
            trigger: p && p.trigger === "postCompaction" ? "postCompaction" : "everyTurn"
          }));
    ...
    function PIForm(scope) {
      this.scope = scope; // 0.1.7：ConfigFormController（旧 settingsScope 同形替代）
      this.staged = new Map();
    ...
    const zh = {
      ...
      "card.description": "每轮对话向模型上下文注入你配置的提示词（每条一行折叠提醒，界面自动加「上下文注入」前缀）；可按需开关、添加/删除/修改提示词。",
      "card.statusOn": "启用",
      "card.statusOff": "停用",
      ...
      "field.prompts": "提示词列表",
      "hint.prompts": "每条启用中的提示词按触发模式注入折叠提醒：每轮注入=每一轮对话一条；仅压缩后=上下文压缩发生后的下一轮实义输入注入一次（同一次压缩不重复）。行首「上下文注入」前缀由界面自动添加，行标题即你的提示词标题。标题用于识别，正文是注入内容。",
      "prompt.triggerLabel": "触发",
      "prompt.triggerEveryTurn": "每轮注入",
      "prompt.triggerPostCompaction": "仅压缩后一次",
      "prompt.enabled": "启用此行",
      "prompt.add": "＋ 添加提示词",
      "prompt.delete": "删除",
    ...
    const en = Object.assign({}, zh, {
      ...
      "card.description": "Inject your configured prompts into the model context every round (one collapsed notice line each; the 「上下文注入」 prefix is added by the UI); toggle on/off, add/remove/edit prompts.",
      ...
      "hint.prompts": "Each enabled prompt injects a collapsed notice per its trigger: every-turn = one per round; post-compaction only = once on the next substantive round after a context compaction (no repeat within the same compaction). The leading 「上下文注入」 is added by the UI; row title is for recognition, body is the injected text.",
      "prompt.triggerLabel": "Trigger",
      "prompt.triggerEveryTurn": "Every turn",
      "prompt.triggerPostCompaction": "Once after compaction",
    ...
    function PromptsEditor(props) {
      ...
      const append = () => {
        ...
        const next = list.concat([{ id, title: "", text: "", enabled: true, trigger: "everyTurn" }]);
        props.edit("prompts", next);
      };
      const rows = list.map((p, i) => {
        ...
        return jsxs("div", { className: "PInj_row", key: key, children: [
          jsxs("div", { className: "PInj_rowHead", children: [
            jsx("input", { className: "PInj_input", value: p.title || "", placeholder: t("prompt.titlePlaceholder"), disabled: !writable, onChange: (e) => cycle(i, { title: e.target.value }) }),
            jsxs("label", { className: "PInj_rowToggle", children: [
              jsx("input", { type: "checkbox", className: "PInj_check", checked: p.enabled !== false, disabled: !writable, onChange: (e) => cycle(i, { enabled: e.target.checked }) }),
              t("prompt.enabled")
            ] }),
            jsxs("label", { className: "PInj_rowTrigger", children: [
              jsx("span", { className: "PInj_triggerLabel", children: t("prompt.triggerLabel") }),
              jsx("select", { className: "PInj_trigger", value: p.trigger === "postCompaction" ? "postCompaction" : "everyTurn", disabled: !writable, onChange: (e) => cycle(i, { trigger: e.target.value }), children: [
                jsx("option", { value: "everyTurn", children: t("prompt.triggerEveryTurn") }),
                jsx("option", { value: "postCompaction", children: t("prompt.triggerPostCompaction") })
              ] })
            ] }),
            jsx("button", { className: "PInj_del", disabled: !writable, onClick: () => remove(i), children: t("prompt.delete") })
          ] }),
          jsx("textarea", { className: "PInj_text", value: p.text || "", rows: 4, placeholder: t("prompt.textPlaceholder"), disabled: !writable, onChange: (e) => cycle(i, { text: e.target.value }) }),
          !(p.text || "") ? jsx("p", { className: "PInj_rowWarn", children: t("prompt.emptyBodyWarn") }) : null
        ] });
      });
      return jsxs("div", { className: "PInj_prompts", children: [
        rows,
        jsx("button", { className: "PInj_add", type: "button", disabled: !writable, onClick: append, children: t("prompt.add") })
      ] });
    }
    ...
    function PromptInjectorCard(props) {
      ...
      // 0.1.7 插件页双视图：列表卡只渲染一行描述；详情页渲染完整表单
      // （官方 AgentLoopCard 同款分支）。列表页由框架把整卡渲染进卡片描述区。
          ...
          group.keys.map((key) => {
            ...
            if (field.type === "list") {
              return jsxs("div", { className: "PInj_field", children: [
                ...
                jsx(PromptsEditor, { t: t, edit: props.edit, list: fv.stagedList, writable: shell.writable }),
      ...
      return jsxs("div", { className: shell.dirty ? "PInj_card PInj_cardOpen" : "PInj_card", children: [
    ...
    // base/revision/mode 字段——PIForm 只读 status/value/user/writable，全兼容）。
    const injectServices = ["slots", "locale", "configForms"];
    ...
    function apply(ctx) {
      ...
      // 的同形替代）；宿主未提供该命名空间时 status 停在 unavailable，卡片自行只读。
      const form = new PIForm(ctx.configForms.get(NS));
      const t = ctx.locale.bind(NS);
      ctx.effect(() => form.scope.subscribe(() => form.publish()), "dsh-prompt-injector: scope-follow");
      // 0.1.7 slots.inject 契约：callback 是「返回 disposer 的普通函数」——
      // 0.1.5 的 generator（function* + yield register）形态已废弃，传 generator
      // 会被 ctx.effect 当普通回调调用，register 永不执行 → 卡片不出现。
      // whileServed：宿主开始服务本命名空间（ns 出现在 describe 视图）才注册，
      // 命名空间消失（插件停用）时自动摘除——官方 agent-loop 同款接线。
      // label thunk：列表页/详情页标题随 locale 现读。
      ctx.effect(
        ...
        "dsh-prompt-injector: card registration"
      );
    }
