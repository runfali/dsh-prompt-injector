window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      "field.prompts": "提示词列表",
      "hint.prompts": "每条启用中的提示词，每一轮对话注入一条折叠提醒：行首「上下文注入」前缀由界面自动添加，行标题即你的提示词标题。标题用于识别，正文是注入内容。",
      "prompt.enabled": "启用此行",
      "prompt.add": "＋ 添加提示词",
      "prompt.delete": "删除",
    ...
    const en = Object.assign({}, zh, {
      ...
      "hint.prompts": "Each enabled prompt is injected every round as one collapsed notice: the leading 「上下文注入」 is added by the UI, and the row title is your prompt title. Title is for recognition; body is the injected text.",
