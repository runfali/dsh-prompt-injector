window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // whileServed：宿主开始服务本命名空间（ns 出现在 describe 视图）才注册卡片，
      // 命名空间消失（插件停用）时自动摘除——0.1.7 官方插件的标准接线。
      // label thunk：插件列表页/详情页标题随 locale 现读（官方 agent-loop 同款）。
      ctx.effect(
        () => ctx.configForms.whileServed([NS], () => ctx.slots.inject("plugins.item", function* () {
